...
STRONG_ANCHORS = [
    ...
    "sous traitement", "sous", "traite par", "medicament", "medication",
    "posologie", "ordonne", "prescrit", "prescription", "remplace par", "remplace",
    "cesse", "cesser", "comme medicament",
    "liste de medicaments", "sur ordonnance",
]
ANCHOR_RE = re.compile(
    r"(" + "|".join(sorted(STRONG_ANCHORS, key=len, reverse=True)) + r")", re.I)
        ...
        while i < n:
            ...
            if base_score >= 0.99 and not is_leaf:
                ...
                score = (PHONETIC_WEIGHT
                         + (ANCHOR_WEIGHT if (has_anchor or region_credit) else 0)
                         + (POSOLOGY_WEIGHT if has_poso else 0))
                if not (has_anchor or has_poso or region_credit):
                    score = 0                   # narrative gate
                elif is_leaf and score < PHONETIC_WEIGHT + POSOLOGY_WEIGHT:
                    score = 0                   # leaves need posology (or strong anchor)
