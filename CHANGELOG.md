...
- **Affichage.** Le panneau « Informations techniques » ajoute « pré-traitement déterministe / modèle (1er jeton) / total » (fr/en, `debug.compute`) ; l'onglet admin « Statistiques » joint `deterministic_seconds` et `ttft_seconds` à chaque ligne du journal de génération.
- Colonnes ajoutées automatiquement (migration `_ADDED_COLUMNS` : `compute_stats_json`, `normalized_transcript`, `inline_fixed_json`).
