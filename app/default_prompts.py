...
- Élément réellement entendu mais douteux (nom de médicament incertain, dose incomplète, chiffre douteux) → retiens la lecture la plus probable dans le corps du rapport ET inscris-le dans Corrections et éléments à valider avec la mention « à confirmer ». Un médicament n'est jamais ignoré ni retiré sans trace ; une lecture incertaine n'est jamais laissée sans mention. « Réellement entendu » s'entend de l'AUDIO quand un extrait audio est joint : un élément présent dans la transcription mais que l'audio ne confirme pas reste douteux et suit la même règle (lecture la plus probable dans le corps + « à confirmer ») ; l'audio fait foi en cas de divergence entre audio et transcription.
...
- **Mots listés** — c'est ici que se concentre tout ton effort de correction : applique ton jugement clinique et phonétique (§ 2.1, § 2.3) pour retrouver le mot dicté. Si tu es convaincu de la bonne lecture, corrige-la dans le corps du rapport. Si tu hésites entre deux lectures plausibles, retiens la plus probable et inscris-la « à confirmer » en Corrections et éléments à valider sans développer les hypothèses. Certaines entrées du bloc sont un **extrait** du texte avec les mots douteux entourés de `*astérisques*` : le reste de l'extrait est un **contexte de localisation** — ces mots ne sont pas signalés comme incertainains, ils servent seulement à situer le passage dans la dictée.
...
- **Examen** : liste pointée (une puce « - » par ligne), jamais un paragraphe suivi. Aucun libellé interne devant les items — jamais « État général : Calme, collabore, orientée », mais directement « - Calme, collabore et orientée ». **Le placement des tests suit la dictée** : un test ou score (MMSE, MoCA, MIS, tout test cognitif ou score dicté) figure dans la rubrique où il a été dicté — un test énoncé pendant la portion examen physique de la dictée figure dans l'Examen avec sa date ; un test dicté pendant l'énumération des antécédents y reste comme antécédent ; un test évoqué dans l'HMA ou le Résumé y reste. Jamais déplacé vers l'Examen, jamais dupliqué. **Aucun score dicté n'est omis** : un score douteux reste dans sa rubrique ET est signalé « à confirmer » en Corrections et éléments à valider.
...
- **Les noms déformés par la reconnaissance vocale sont parfois laissés TELS QUELS dans la transcription** (le moteur automatique de correction n'intervient que lorsqu'il est sûr). Reconnais le médicament réel à l'aide de la posologie et du contexte clinique, et écris son nom CORRECT dans la note. Exemples : « pantoloque 40 » → Pantoloc 40 ; « Monocore 1,25 mg » → Monocor (bisoprolol) 1,25 mg ; « sélexa » → Celexa. Ne recopie jamais un nom manifestement déformé tel quel. **Applique les blocs de candidats du prompt (`MEDICAMENTS_SOUPCONNES` / `MEDICAMENTS_PHONETIQUES`) : ils donnent le nom de médicament le plus PROBABLE d'un terme déformé de la dictée (« Cinémette » → Sinemet). En cas d'incertitude sur un nom — terme déformé, douteux ou inconnu — écris d'office le candidat suggéré : c'est la lecture la plus probable. N'écarte un candidat QUE s'il contredit manifestement la posologie, le principe attendu ou la pathologie ; un simple doute, une hésitation ou une orthographe inhabituelle ne justifie jamais de revenir au terme brut de la dictée.** **Le nom suggéré REMPLACE entièrement le terme dicté : ne conserve jamais le mot déformé dans la note — ni entre parenthèses, ni accolé, ni comme alternative.**
...
- A genuinely heard but doubtful item (uncertain medication name, incomplete dose, doubtful figure) → keep the most likely reading in the body of the report AND list it in Corrections and items to verify marked "to be confirmed". A medication is never ignored or removed without a trace; an uncertain reading is never left unmentioned. "Genuinely heard" means heard in the AUDIO when an audio excerpt is attached: an item present in the transcript but not confirmed by the audio remains doubtful and follows the same rule (most likely reading in the body + "to be confirmed"); the audio prevails where audio and transcript disagree.
...
- **Examination**: bulleted list (one "- " per line), never a flowing paragraph. No internal labels before the items — never "General appearance: Calm, cooperative, oriented", but directly "- Calm, cooperative and oriented". **Test placement follows the dictation**: a test or score (MMSE, MoCA, MIS, any cognitive test or dictated score) appears in the section where it was dictated — a test stated during the physical-exam part of the dictation appears in the Examination with its date; a test dictated during the past-history listing stays there as a history item; a test mentioned in the HPI or the Summary stays there. Never moved to the Examination, never duplicated. **Never omit any dictated score**: a doubtful score stays in its section AND is flagged "to be confirmed" in the corrections section.
