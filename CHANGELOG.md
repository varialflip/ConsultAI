...
- **Largeur portée à 91 caractères** (`LINE_WIDTH`) : deux caractères
  restaient disponibles dans la largeur du dossier médical. Les colonnes de la
  liste pointée deux colonnes restent à 44 (`MEDS_COLUMN_WIDTH`, partagée avec
  les médicaments), pour un total de 44 + 2 + 44 = 90 qui tient sous la marge
  (un total de 92 débordait et cassait la liste des médicaments).
...
- **Merge de `origin/main`** (7 commits, 2026-08-28 → 09-04) dans `selfhosted` :
  ...
  lectures douteuses.
- **Tableaux en boîte Unicode pleine largeur dans « Aligné »** (`renderUnicodeTable`) :
...
- **Un seul scan plein texte en cas de course « Terminer » → « Générer ».** La génération ATTEND maintenant (bornée, ~30 s) le scan de fond déjà armé par le « Terminer` (`_grounding_events`) plutôt que d'en lancer un second synchrone : même résultat, et les ~10-17 s de phonétique ne sont plus payées deux fois au pire moment. Le scan synchrone ne sert plus que de filet (aucun scan de fond).
- **Statistiques de calcul dans les métadonnées de la consultation** (`compute_stats_json`) : durées du scan plein texte et du pré-calcul (« Terminer ») + durées déterministes de la génération (audio, normalisation avec `normalize_source` precomputed/compute, gériatrique, mots douteux) + télémétrie LLM (`llm_ttft_ms` — premier jeton — et `llm_total_ms`). Timings et compteurs uniquement, aucune donnée clinique.
- **Affichage.** Le panneau « Informations techniques » ajoute « pré-traitement déterministe / modèle (1er jeton) / total » (fr/en, `debug.compute`) ; l'onglet admin « Statistiques » affiche `deterministic_seconds` et `ttft_seconds` à chaque ligne du journal de génération — valeurs figées SUR L'ÉVÉNEMENT (`usage_events.compute_stats_json`) pour montrer chaque génération telle qu'elle s'est déroulée, et non la dernière connue de la consultation.
- Colonnes ajoutées automatiquement (migration `_ADDED_COLUMNS` : `compute_stats_json`, `normalized_transcript`, `inline_fixed_json` sur `consultations` ; `compute_stats_json` sur `usage_events`).
